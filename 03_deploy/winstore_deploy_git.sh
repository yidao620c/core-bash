...
[[ ! -d winstore-ansible ]] && git clone git@192.168.217.161:xiongneng/winstore-ansible.git
echo "先回退最新的"
cd winstore-ansible
echo "先从gitlab拉取最新源代码"
git checkout -- install_mysql.sh install_winstore.sh
...
if [[ "$#" > 1 && "$2" == "mysql" ]]; then
    ...
    cd ../..
    new_file_name=mysql_install_$(date +"%Y%m%d").tar.gz
