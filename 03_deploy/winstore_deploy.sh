...
if [[ "$#" > 1 && "$2" == "mysql" ]]; then
    ...
    cd ../..
    new_file_name=winstore"$1"_mysql_$(date +"%Y%m%d").tar.gz
    tar -cpzvf ${new_file_name}  winstore-ansible/install_mysql.sh winstore-ansible/mysql.yml winstore-ansible/group_vars winstore-ansible/install_ansible winstore-ansible/resource/install_mysql_rpms.tar.gz winstore-ansible/resource/shell/mysql.sh 
    echo "打完包后删除里面的tar包"
